...
# Set ProObejct Env ##############################################
if [ -z $PORTE_ENV ]; then PORTE_ENV=true; fi
if [ $PORTE_ENV = "true" ] && [ -e "${SCRIPT_HOME}/env_prte.sh" ]; then
 . ${SCRIPT_HOME}/env_prte.sh
fi
##################################################################
...
# Set SCF ID NAME ################################################
if [ -z $SCF_ID ]; then SCF_ID="axa-sample"; fi
...
sed -i "s,%JEUS_HOME%,${JEUS_HOME},g" ${DOMAIN_HOME}/config/domain.xml

if [ -z $BASE_ADDRESS ]; then export BASE_ADDRESS=`uname -n`; fi
sed -i "s/%BASE_ADDRESS%/${BASE_ADDRESS}/g" ${DOMAIN_HOME}/config/domain.xml

if [ -z $SERVERNAME ]; then export SERVERNAME=`uname -n`; fi
